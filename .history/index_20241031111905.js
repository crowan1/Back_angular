...
app.get('/tickets', (req, res) => {
    connection.query('SELECT * FROM tickets', (err, results) => {
        ...
        const formattedResults = results.map(ticket => {
            return {
                ...ticket,
                created_at: moment(ticket.created_at).tz('Europe/Paris').format('YYYY-MM-DD HH:mm:ss'), // Format sans T
                resolved_at: ticket.resolved_at ? moment(ticket.resolved_at).tz('Europe/Paris').format('YYYY-MM-DD HH:mm:ss') : null
            };
        });
