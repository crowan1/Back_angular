...
app.put('/tickets/:id', (req, res) => {
    ...
    const { description, priority, status, assigned_to } = req.body;

    const query = 'UPDATE tickets SET description = ?, priority = ?, status = ?, assigned_to = ? WHERE id = ?';
    connection.query(query, [description, priority, status, assigned_to, ticketId], (err, results) => {
        if (err) return res.status(500).json({ error: err });
