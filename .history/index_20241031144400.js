...
app.get('/tickets', (req, res) => {
    connection.query('SELECT t.*, u.email AS assigned_to FROM tickets t LEFT JOIN utilisateur u ON t.assigned_to = u.id', (err, results) => {
        ...
        const formattedResults = results.map(ticket => {
            // Ajustez le timestamp pour le fuseau horaire (UTC+1 ou UTC+2)
            const createdAt = new Date(ticket.created_at);
            const resolvedAt = ticket.resolved_at ? new Date(ticket.resolved_at) : null;

            // Ajoutez 1 ou 2 heures selon le besoin
            createdAt.setHours(createdAt.getHours() + 1); // Utilisez +2 si vous êtes en heure d'été
...
app.post("/createtickets", authenticateToken, (req, res) => {
    ...
    // Ajout de 1 heure pour le fuseau horaire
    const query = "INSERT INTO tickets (description, priority, created_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 1 HOUR))";
