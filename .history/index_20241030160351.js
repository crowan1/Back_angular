...
app.get("/administration", authenticateToken, (req, res) => {
    connection.execute(
      `SELECT id, nom, email FROM utilisateur`, // Remplacez `nom` par le champ qui correspond au nom dans votre table
      (err, rows) => {
        if (err) {
        ...
        // Si la requête réussit, renvoyer les données des utilisateurs
