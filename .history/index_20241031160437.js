...
app.put("/tickets/:id", authenticateToken, (req, res) => {
    const { id } = req.params;
    const { description, priority, assignedTo, status } = req.body;

    const query = `
        UPDATE tickets 
        SET description = ?, priority = ?, assigned_to = ?, status = ? 
        WHERE id = ?
    `;
    
    connection.execute(query, [description, priority, assignedTo, status, id], (err, results) => {
        if (err) {
            console.error("Erreur lors de la mise à jour du ticket:", err);
            return res.status(500).json({ error: "Erreur lors de la mise à jour" });
        }

        res.status(200).json({ id, description, priority, assignedTo, status });
    });
});
